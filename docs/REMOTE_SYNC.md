...
編集端末は文章、確定文、日本語変換状態、消し跡、設定、セッション状態を1つのスナップショットとして送る。連続入力は200ms単位でまとめる。削除、設定変更、一時停止、新しい文章への切替も同じスナップショットに含まれる。32KB以上のスナップショットはブラウザでgzip圧縮し、Base64文字列を `snapshot_gzip` として送る。Durable Objectは圧縮状態のまま表示端末へ中継・保存し、表示端末が展開する。古いクライアントの通常 `snapshot` も受け付ける。

Durable Objectは最新スナップショットを接続中の全表示端末へすぐ配信し、1秒単位で保存する。途中参加・再接続した表示端末にはメモリ上または保存済みの最新版を送る。履歴全体はサーバーへ送らない。

## 表示面の寸法と自動追随

表示面の横幅は、端末のviewport幅と基準紙面 `960 × 620` を画面内へ収める幅のうち、小さいほうを使う。文章が長くなって紙面の論理高が増えても横幅は縮めない。これにより、文字数が増えたときに文字サイズまで小さくなることを防ぐ。

短い文章はこれまでどおり画面の上下中央へ置く。紙面がviewport高を超えた場合だけ表示面を縦スクロール可能にする。スクロールバーは投影に出さず、マウスホイール、トラックパッド、タッチ操作で動かせる。

編集スナップショットにはtextareaの `selectionEnd` を含める。表示端末はその文字位置に対応する行を求め、画面の上から約68%の位置へ自動スクロールする。カーソル移動だけでもスナップショットを更新するため、文章の末尾以外を推敲している場合も編集中の行へ追随する。文章全体が1画面に収まる場合はスクロール位置を先頭へ戻し、中央配置を保つ。

## 文字組みと中央配置

編集面と表示面は `measurePaperText` の同じ計測結果を使う。ブラウザ上で実際のフォント、字間、自動行高、折り返し位置を測り、短い文章は字形全体が基準紙面 `960 × 620` の縦中央に来るよう最初のベースラインを決める。横位置は文字揃えの既定値である中央揃えを使う。文章が中央の領域に収まらない場合は、上下に余白を足して紙面の論理高を伸ばす。

行高は既定の76pxと文字サイズの112%のうち大きいほうを自動で使う。textarea、折返し計測、Canvasの消し跡で同じ行高とベースラインを共有し、最大300pxでも字形の上端・下端が切れないようにする。行詰めの手動設定は中央配置を崩す原因になったため、設定画面と保存データから除外している。

## 編集権の排他制御

同じ編集URLを複数端末や複数ウィンドウで開いても、最初に認証された1接続だけを編集者として扱う。後から接続した編集画面には `editor_conflict` を返し、再接続ループを止めて入力、タイマー、履歴削除、表示設定の変更を無効にする。先に接続している編集画面はそのまま動作を続ける。同じ画面が通信断から再接続した場合は、認証時の `clientId` で判定して古い接続を閉じ、新しい接続を許可する。別画面の `clientId` は競合扱いのまま。
...
上演状態は最終利用から24時間を目安に失効させる。1メッセージは1MB以下。WorkerはGitHub Pages本番URLとローカル開発URLからの接続だけを受け付ける。

### 長時間の推敲で起きた切断

従来は消し跡 `marks` を毎回すべてJSONで送っていた。1文字を消すたび約170バイト増え、約1,500字分の消し跡でアプリ独自の256KB上限に達する。WorkerがWebSocketをcode `1009` で閉じても、クライアントは同じ大きなスナップショットを再送したため切断が繰り返された。これはCloudflare本体の32MiB上限ではなく、アプリが設けた上限だった。3,000個の消し跡を含む例は非圧縮約510KBからgzip + Base64で約90KBとなる。保存済みの消し跡・CSVには変更を加えない。

将来、圧縮後でも1MBを超えた場合は自動再接続を繰り返さず接続エラーとして止める。編集端末のローカル保存は継続する。

加えて、一時的な通信断の再接続時、Durable Object側に同一画面の古いWebSocketがまだ残っていると競合判定される余地があった。`clientId` による同一画面の再接続を追加し、別画面の編集排他は維持する。
...
- 縦長・横長の表示端末で横幅が画面に収まり、文章量が増えても文字が縮小しない。
- 長文では手動スクロールでき、入力またはカーソル移動後に編集中の行へ追随する。
...
- 2026-09-21: 長時間の推敲で消し跡が増えると256KB制限で切断される問題を調査。gzipスナップショットと同一画面の再接続判定を追加。
- 2026-09-21: 行詰めの手動設定を削除。過去にlocalStorageへ保存された行詰め値も読み込まず、文字サイズに応じた自動行高へ統一。
- 2026-09-21: 文字サイズと行数から字形領域を計測し、短い文章を紙面の縦中央へ配置。最大文字サイズでも字形が紙面上端や隣の行で切れないよう、文字サイズに応じた最低行高を編集面と表示面の共通レイアウトへ追加。
- 2026-09-20: 表示面の幅を文章の高さから分離。長文を縦スクロール化し、編集位置への自動追随を追加。
