...
編集面と表示面は `measurePaperText` の同じ計測結果を使う。ブラウザ上で実際のフォント、字間、行間、折り返し位置を測り、短い文章は字形全体が基準紙面 `960 × 620` の縦中央に来るよう最初のベースラインを決める。横位置は文字揃えの既定値である中央揃えを使う。文章が中央の領域に収まらない場合は、上下に余白を足して紙面の論理高を伸ばす。

文字サイズが大きく、指定された行間では隣の行と重なる場合、描画に使う実効行高を文字サイズの112%以上にする。設定値自体は変更せず、指定行間のほうが広ければその値を使う。textarea、折返し計測、Canvasの消し跡で同じ実効行高とベースラインを共有し、最大300pxでも字形の上端・下端が切れないようにする。
